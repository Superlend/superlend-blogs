...
export function FeaturedPostCard({
  ...
  return (
    <article
      className={`group relative overflow-hidden rounded-2xl ${cardHeight}`}
    >
        ...
        <div className="relative h-full flex flex-col justify-between p-6 text-white">
          ...
          <div className="flex items-start justify-between">
            ...
            {isFeatured && (
              <Badge
                variant="secondary"
                className="bg-white/50 backdrop-blur-sm text-yellow-800/75 border-white/30"
              >
                ⭐ Featured
              </Badge>
            )}
          ...
          <div className="space-y-4">
            <h3
              className={`font-bold leading-tight text-white transition-all duration-300 ease-out group-hover:text-emerald-300 group-hover:transform group-hover:translate-y-[-2px] ${
                isSmall ? "text-lg" : "text-2xl lg:text-3xl"
              }`}
            >
              {title}
            </h3>
